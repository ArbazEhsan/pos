...
<div class="container">
  <form method="POST" id="formId">
    <div class="row">
      ...
      <div class="col-md-2">
        <input type="number" class="form-control" id="unit" placeholder="Liter/Gram" disabled>
      </div>
      <div class="col-md-3">
  ...
  <div class="row">
      <div class="col-md-12">
        <table class="table table-bordered table-striped">
        	...
        	<tbody>
        		<?php 
        			$result11 = mysqli_query($con,"SELECT * FROM finished ORDER BY day");
        			while($fetch11=mysqli_fetch_array($result11)) {
        			$consume = '';
        			$pro = explode(',', $fetch11['consume']);
				    foreach ($pro as $key => $value) {
				  	  $result12 = mysqli_query($con,"SELECT * FROM products WHERE id='$pro[$key]'");
				  	  $fetch12 = mysqli_fetch_array($result12);
				  	  $consume .= $fetch12['name'].',';
				    }
				    $result13 = mysqli_query($con,"SELECT * FROM products WHERE id='".$fetch11['finish_good']."'");
				  	$fetch13 = mysqli_fetch_array($result13);
        		?>
        		<tr>
        			<td><?php echo date('d-M-Y', strtotime($fetch11['day'])); ?></td>
        			<td><?php echo $fetch13['name']; ?></td>
        			<td><?php echo $fetch11['fQty']; ?></td>
        			<td><?php echo $consume; ?></td>
        			<td><?php echo $fetch11['qty']; ?></td>
        			<td><a href="finished.php?delId=<?php echo $fetch11['id']?>&consume=<?php echo $fetch11['consume'] ?>&qty=<?php echo $fetch11['qty'] ?>&fg=<?php echo $fetch11['finish_good'] ?>&fQty=<?php echo $fetch11['fQty'] ?>" class="btn btn-danger">Delete</a></td>
        		</tr>
        		<?php } ?>
        	</tbody>
        </table>
