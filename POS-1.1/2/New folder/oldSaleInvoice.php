"SELECT psale.barcode, sale.price, sale.qty, sale.finalValue, accounts.name, accounts.city, products.name AS pname FROM psale INNER JOIN sale ON psale.barcode=sale.barcode INNER JOIN accounts ON psale.customer=accounts.id INNER JOIN products ON psale.barcode=products.id WHERE psale.customer='$product' AND psale.sale_day BETWEEN '$fromDate' AND '$toDate'"
            ...
            <input list="group" autofocus id="customer" name="customer" class="form-control" autocomplete="off" value="<?php echo $fetch['name']; ?>" placeholder="Choose Customer" required>
                     <datalist id="group">
                      ...
                      <ul class="dropdown-menu" role="menu">
                                   <?php
                                    $sql="SELECT * FROM accounts WHERE active !='0' && type!='Vendor'";
                                    $result=mysqli_query($con,$sql);
...
<!-- <nav aria-label="..." style="position: fixed;">
  ...
  <h5>Here you can generate invoices</h5><hr>
      ...
      <form method="POST" id="lockForm">
        <div class="col-md-2">
          ...
          <div class="col-md-3">
            ...
            ?>
            <h4>Customer Name</h4><!-- onchange="lock()" --> 
            <select autofocus id="customer" name="customer" class="form-control" value="<?php echo $fetch['name']; ?>" required>
              <?php
    ...
    <button  type="button" id="modl" class="form-control" data-toggle="modal" data-target="#myModal" onclick="alertt()" accesskey="o" style="display:none;">Click To Open Search Form</button>
