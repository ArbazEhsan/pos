...
$orderno = $_REQUEST['id'];
$result = mysqli_query($con,"SELECT * FROM pcounter WHERE id='$orderno'");
...
<div class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-md-2">
      ...
      <input type="text" name="vendor_bilty_No" id="vendor_bilty_No" class="form-control" autofocus placeholder="Vendor Bill No" value="<?php echo $fetch['bill_No']; ?>">
    </div>
    <div class="col-md-2">
      <label>Bilty Date</label>
      <input type="date" name="bilty_date" id="bilty_date" class="form-control" placeholder="Bilty Date" value="<?php echo $fetch['bill_date']; ?>">
    </div>
    <div class="col-md-2">
      <label>Bilty No</label>
      <input type="text" name="bilty_No" id="bilty_No" class="form-control" placeholder="Bilty No" value="<?php echo $fetch['bilty_No']; ?>">
    </div>
    <div class="col-md-2">
      <label>Reference</label>
      <input type="text" name="reference" id="reference" class="form-control" placeholder="Reference" value="<?php echo $fetch['transport_By']; ?>">
    </div>
    <div class="col-md-2">
      <label>A/C Title</label> 
      <select id="customer" name="customer" class="form-control" autofocus required>
        <?php
        $result2=mysqli_query($con,"SELECT * FROM accounts WHERE active !='0' && type!='Customer' ORDER BY name");
    ...
    <div class="col-md-12">
      <table class="table table-bordered table-striped">
        ...
        <tbody id="item-details">
          ...
          <td><input type="text" class="form-control qty" name="qty[]" id="qty" placeholder="Qty" autocomplete="off" value='<?= $fetch3['qty']; ?>'></td>
          <td><input type="text" class="form-control price" name="price[]" id="price" placeholder="P.Price" autocomplete="off" value='<?= $fetch3['price']; ?>'>
          <td><input type="text" class="form-control wprice" name="wprice[]" id="wprice" placeholder="W.Price" value='<?= $fetch3['w_price']; ?>'></td>
          <td><input type="text" class="form-control rprice" name="rprice[]" id="rprice" placeholder="R.Price" autocomplete="off" value='<?= $fetch3['r_price']; ?>'></td>  
          <td><input type="text" class="form-control total" name="total[]" id="total" placeholder="Total" autocomplete="off" value='<?= $fetch3['finalValue']; ?>' readonly></td>
...
$(document).ready(function () {
  $('#save').click(function () {
    ...
    if(!$("#lockForms")[0].checkValidity()){
      ...
      $.ajax({
          url: "operation.php?from=purchaseInvoice&operation=update",
          type: 'POST',
          data: formData,
          async: false,
  ...
  function append(argument) {
    ...
    var newRow="";
      ...
      '<td><input type="text" class="form-control price" name="price[]" id="price'+n+'" placeholder="P.Price" autocomplete="off">'+
      '<td><input type="text" class="form-control wprice" name="wprice[]" id="wprice'+n+'" placeholder="W.Price"></td>'+
      '<td><input type="text" class="form-control rprice" name="rprice[]" id="rprice'+n+'" placeholder="R.Price" autocomplete="off"></td>'+  
