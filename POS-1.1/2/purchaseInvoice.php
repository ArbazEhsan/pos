...
<div class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-md-2">
      ...
      <input type="text" name="vendor_bilty_No" id="vendor_bilty_No" class="form-control" autofocus placeholder="Vendor Bill No">
    </div>
    <div class="col-md-2">
      <label>Bilty Date</label>
      <input type="date" name="bilty_date" id="bilty_date" class="form-control" placeholder="Bilty Date">
    </div>
    <div class="col-md-2">
      <label>Bilty No</label>
      <input type="text" name="bilty_No" id="bilty_No" class="form-control" placeholder="Bilty No">
    </div>
    <div class="col-md-2">
    ...
    <div class="col-md-2">
      ...
      <select id="customer" name="customer" class="form-control" value="<?php echo $fetch['name']; ?>" required>
        <?php
        $result=mysqli_query($con,"SELECT * FROM accounts WHERE active !='0' && type!='Customer' ORDER BY name");
...
$(document).ready(function () {
  $('#save').click(function () {
    ...
    if(!$("#lockForms")[0].checkValidity()){
      ...
      $.ajax({
          url: "operation.php?from=purchaseInvoice&operation=insert",
          type: 'POST',
          data: formData,
          async: false,
          success: function (info) {
            // alert(info);
            window.open('purchasePrint.php?pur_No='+info,'popUpWindow','height=400,width=1000,left=50,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
            location.reload();
          },
          cache: false,
          contentType: false,
  ...
  function getProductInfo(id,num) {
    $.ajax({
        url: "operation.php?from=purchaseInvoice&operation=show&id="+id,
        type: 'GET',
        async: false,
        success: function (info) {
          // alert(info);
          ...
          $("#wprice"+num).val(data[2]);
          $("#rprice"+num).val(data[3]);
          total();
          append();
        },
        cache: false,
