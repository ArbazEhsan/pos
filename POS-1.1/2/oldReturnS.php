...
<center><h1>Sale Return Menu</h1></center><hr>
        ...
        <input list="group" id="cName" name="cName" class="form-control" placeholder="Choose Customer" onchange="invoice(this.value);" autocomplete="off" required>
          <datalist id="group">
            <?php
              ...
              $sql="SELECT * FROM accounts WHERE type!='Vendor' AND active!='0' ORDER BY name";
...
<script type="text/javascript">
  function invoice(customerName){
    ...
    xhttp.open("GET", "returnAjax.php?from=invoice"+"&customer="+customerName,true);
    xhttp.send();
  }
  function insert(){
    saleNo = $('#saleNo').val();
    ...
    xhttp.open("GET", "queue.php?from=SR&saleNo="+saleNo,true);
    xhttp.send();
  }
