...
<div class="row">
	<table border="1" class="table" id="myTable" width="100%">
		...
		// $result3 = mysqli_query($con,"SELECT *,SUM(qty) AS qty1 FROM purchase WHERE pur_day BETWEEN '$today' AND '$fromday'");
		    $date = date('Y-m-d');
			$result4 = mysqli_query($con,"SELECT SUM(amount) AS sumAmnt FROM cashout WHERE day='$date'");
			...
			$result9 = mysqli_query($con,"SELECT SUM(amount) AS sumAmnt1 FROM cashin WHERE day='$date'");
