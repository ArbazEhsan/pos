...
<div class="row">
	<div class="col-md-4">
		<button class="btn btn-primary" name="btn">Generate Report or All</button>
...
<div class="row">
    ...
    //         $id = $fetch1['id'];		 	
		 	...
		 	$sql = "SELECT ledgers.*,accounts.*,SUM(ledgers.cr) AS credit,SUM(ledgers.dr) AS debit FROM ledgers INNER JOIN accounts ON ledgers.account_id = accounts.id WHERE accounts.type='Vendor' GROUP BY ledgers.account_id";
		 ...
		 elseif($fromday!='' && $today!='') {		 	
		 	...
		 	$sql = "SELECT ledgers.*,accounts.*,SUM(ledgers.cr) AS credit,SUM(ledgers.dr) AS debit FROM ledgers INNER JOIN accounts ON ledgers.account_id = accounts.id WHERE customer.type='Vendor' AND ledgers.day BETWEEN '$fromday' AND '$today' GROUP BY ledgers.account_id";
		 	$result1 = mysqli_query($con,$sql);
		 }
