...
if(isset($_REQUEST['delId']))
{
  ...
  header("location:viewCashin.php");
}
 ?>
<title>View Cash In</title>
<?php
include ("../header.php");
...
<body>
<?php 
		$result = mysqli_query($con,"SELECT * FROM tcounter WHERE type='CR' ORDER BY day");
 ?>
...
<center><h1>View Cashin</h1></center><hr>
  ...
  <div class="row">
    ...
    <tfoot>
      ...
      <td><button class="btn btn-primary" onclick="view(this.id)" id="<?php echo $fetch['id'] ?>">View Invoice</button>
        <a href="editCash.php?from=CR&id=<?php echo $fetch['id']?>&vno=<?php echo $fetch['voucher_no'] ?>" class="btn btn-warning">Edit</a>
        <a href="viewCashin.php?delId=<?php echo $fetch['id']?>" class="btn btn-danger" onclick="return func()">Delete</a>
        </td>
 		</tr>
    <?php }?>
    </tbody>
