    ...
    <?php
        ...
        $result4 = mysqli_query($con,$sql1);
         while($fetch4 = mysqli_fetch_array($result4)){
            ...
            if($fetch4['type']=='SV' || $fetch4['type']=='RS' || $fetch4['type']=='PV'){
            ...
            if($fetch4['type']!='CR' && $fetch4['type']!='CO'){
              ...
              if ($fetch4['type']=='RS') {
                $sql2 = "SELECT returnsale.qty, returnsale.price, products.name FROM returnsale INNER JOIN products ON returnsale.barcode=products.id WHERE returnsale.sale_No='".$fetch4['invoice_id']."'";
...
function popup(from,info) {
  if(from=='SV') {
    window.open('../2/salePrint.php?sale_No='+info,'popUpWindow','height=500,width=500,left=50,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
  } else if(from=='CO'){
    ...
    window.open('../2/purchasePrint.php?pur_No='+info,'popUpWindow','height=400,width=1000,left=50,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
  } else if(from=='RP'){
    window.open('../2/returnPPrint.php?sale_No='+info,'popUpWindow','height=400,width=1000,left=50,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
  } 
}
</script>