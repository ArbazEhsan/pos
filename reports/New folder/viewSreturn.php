	...
	<table border="1" class="table" id="myTable" width="100%">
		...
		$customername = $_POST['customername'];	
		 ...
		 elseif ($customername !='' && $fromday!='' && $today!='') {
		 	...
		 	$result2 = mysqli_query($con,"SELECT id,customer,day,SUM(cr) AS credit,SUM(db) AS debit FROM customerledger WHERE customer='$id' AND day BETWEEN '$fromday' AND '$today' GROUP BY customer");
		 }
		...
		{    
			if ($fetch2['credit'] < $fetch2['debit'])
				{
					$counter++;
			 	$total1 = $fetch2['debit'] - $fetch2['credit'];
